...
### 3. BATCH B3: Memory-Only Access Token, Auth Coordinator, Multi-Tab Sync, and WebSocket Security
- **Defect**: Access token remained stored in browser `localStorage`, persisting sensitive credentials across document lifecycles and exposing them to disk storage, offline inspection, and cross-session persistence. Multi-tab refresh, login, and logout races lacked origin-wide serialization. WebSocket transports historically attempted URL query parameter token transmission and lacked bounded 4401 auth re-authentication.
- **Remediation**:
  - **Volatile In-Memory Access Token Storage**: Implemented `frontend/src/lib/auth-token-memory.ts` storing access token strictly in private JavaScript volatile memory. Access token is never written to `localStorage`, `sessionStorage`, `document.cookie`, `IndexedDB`, or `Cache Storage`. Unconditional startup purge removes any legacy `access_token` and `refresh_token` from both `localStorage` and `sessionStorage` without reading, decoding, migrating, or transmitting them.
  - **Deterministic State Machine & Token-Free Zustand**: Refactored `frontend/src/stores/auth.ts` to follow an explicit lifecycle state machine (`idle`, `bootstrapping`, `authenticated`, `refreshing`, `logging_out`, `unauthenticated`). Raw access and refresh tokens are completely absent from Zustand store state.
  - **HttpOnly Cookie Bootstrap**: Application initialization (`bootstrap()` / `hydrate()`) executes exactly once per document attempt. Performs `POST /api/auth/refresh` sending the HttpOnly refresh cookie via `credentials: 'same-origin'`. On 200, stores the new access token in memory, retrieves the authoritative profile via `/auth/me`, and transitions to `authenticated`. On failure, fails closed into `unauthenticated`.
  - **Bounded HTTP 401 Recovery & Request Single Flight**: In `frontend/src/lib/api.ts`, outgoing requests capture the current token `generation`. Concurrent 401 responses in a single tab share one in-flight refresh promise. If the token generation has already changed, callers retry once with the fresh memory token without triggering a duplicate refresh. A secondary 401 expires the session and dispatches `auth:expired` without recursive refresh loops.
  - **Origin-Wide Web Locks & BroadcastChannel Synchronization**: In `frontend/src/lib/auth-coordinator.ts`, cookie-mutating auth operations (`refresh`, `login`, `logout`) are serialized origin-wide using the Web Locks API (`aigold-auth-mutation`) when available. Non-secret lifecycle events (`refresh-started`, `refresh-completed`, `refresh-failed`, `session-changed`, `logout`, `session-expired`) are broadcast across same-origin tabs via BroadcastChannel (`aigold-auth`). Raw tokens, credentials, and cookies are never broadcasted. Fallback race recovery allows a loser tab of an unavoidable concurrent refresh to wait for completion and safely retry once.
  - **Session Epoch & Stale Completion Guard**: Token mutations track monotonic in-memory `sessionEpoch` counters. Asynchronous completions (e.g., refresh finishing after user clicks logout) detect epoch divergence, immediately discard late token responses, and ensure logged-out sessions are never restored.
...
| **Batch B3 Coordinator & In-Memory Token Suite** | In-memory token lifecycle, generation advance, epoch invalidation, single-flight 401 storm, Web Locks serialization, BroadcastChannel synchronization, 4401 WebSocket recovery | **PASS** | 12 passed (`tests/b3-auth-coordinator.test.cjs`) |
| **Frontend Test Suite** | Component, contract, truthfulness, cookie auth, memory token, storage zero-token, coordinator tests | **PASS** | 268 passed, 0 failed (`npm test -- --run`) |
