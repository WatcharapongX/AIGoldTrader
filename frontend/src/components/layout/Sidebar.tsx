...
import { RuntimeHealth, type RuntimeState } from './RuntimeStatus';
...
const navItems = [
  ...
  { href: '/analysis', label: 'Market Analysis', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96.44 2.5 2.5 0 0 1-2.96-3.08 3 3 0 0 1-.34-5.58 2.5 2.5 0 0 1 1.32-4.24 2.5 2.5 0 0 1 1.98-3A2.5 2.5 0 0 1 9.5 2Z"/><path d="M14.5 2A2.5 2.5 0 0 0 12 4.5v15a2.5 2.5 0 0 0 4.96.44 2.5 2.5 0 0 0 2.96-3.08 3 3 0 0 0 .34-5.58 2.5 2.5 0 0 0-1.32-4.24 2.5 2.5 0 0 0-1.98-3A2.5 2.5 0 0 0 14.5 2Z"/></svg> },
...
export function Sidebar({ runtime }: { runtime: RuntimeState }) {
  const pathname = usePathname();
  const user = useAuthStore(state => state.user);
  const logout = useAuthStore((state) => state.logout);

  return (
    <div className="w-14 lg:w-[240px] shrink-0 bg-[#0d1117] h-screen flex flex-col border-r border-gray-800/60 overflow-hidden text-sm">
      {/* Logo Area */}
      <div className="p-3 lg:p-5 flex items-center gap-3">
        ...
        <div className="hidden lg:block font-bold text-lg tracking-tight text-white whitespace-nowrap">
          AIGoldTrader
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-2 space-y-0.5 custom-scrollbar pb-4">
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              title={item.label}
              className={`flex items-center gap-3 px-1 lg:px-3 py-2 rounded-md transition-all ${
                isActive
        ...
        <section className="hidden lg:block px-3 py-2" aria-label="System Status">
          <h2 className="text-gray-300 mb-3">System Status</h2>
          <RuntimeHealth {...runtime} />
        </section>
      </nav>

      {/* User Profile */}
      <div className="p-2 lg:p-4 border-t border-gray-800/60 bg-white/[0.02]">
        <div className="flex items-center justify-between group">
          <div className="hidden lg:flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-medium text-gray-200 leading-tight">{user?.email || "—"}</div>
              <div className="text-xs text-amber-500/80 font-medium">{user?.role || "UNKNOWN"}</div>
            </div>
          </div>
      ...
      {/* Footer Bar */}
      <div className="hidden lg:block py-2 text-center text-[10px] text-gray-600 border-t border-gray-800/40 bg-[#0a0e14]">
        AIGoldTrader
      </div>
    </div>
  );
