...
export function useRuntimeStatus(includeMarket = true): RuntimeState {
  const [state, setState] = useState<RuntimeState>({health:null, ready:null, market:null, system:null});
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      const results = await Promise.allSettled([
        ...
        api.get('/system/status', {signal}) as Promise<SystemStatusResponse>,
      ]);
      if (!abort.signal.aborted) setState({
        ...
        system:results[3].status === 'fulfilled' ? (results[3].value as SystemStatusResponse) : null,
      });
      busy = false;
    };
    void refresh();
...
export function RuntimeHealth({ health, ready, market, system }: RuntimeState) {
  const aiStatus = system?.ai_status_label || (system?.modules?.ai_provider ? system.modules.ai_provider.state : undefined);
