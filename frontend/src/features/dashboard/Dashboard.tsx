...
export function DashboardView({
  ...
  const newsRiskState =
    data?.news?.news_regime === 'NEWS_LOCK' || data?.news?.news_regime === 'RELEASE'
      ? 'BLOCK'
      : data?.news?.news_regime === 'PRE_NEWS' || data?.news?.news_regime === 'POST_NEWS_VOLATILITY' || data?.news?.multiple_event_risk
      ? 'CAUTION'
      : 'NORMAL';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-white font-semibold flex flex-wrap items-center gap-2">
                <span className="text-amber-400">⚡</span> Latest Trading Opportunity
              </h2>
            ...
            {plan ? (
              <>
                ...
                <div className="mt-2 text-[11px] text-gray-500 flex justify-between">
                  <span>สถานะความเสี่ยง: <b className="text-emerald-400">APPROVED (Risk Gate Passed)</b></span>
          ...
          <div className="bg-[#111827] border border-gray-800 rounded-xl p-5">
            ...
            <div className="rounded-xl border border-gray-800 bg-[#0a0f1a] p-4 space-y-3 text-xs">
              <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                <span className="text-gray-400">Meta Controller Verdict:</span>
                <span className="text-white font-semibold">
                  {plan ? (direction === 'LONG' ? 'BULLISH CONCURRENCE' : 'BEARISH CONCURRENCE') : 'STANDBY / AWAITING SETUP'}
                </span>
              </div>
              ...
              <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                ...
                <span className={`font-bold ${direction === 'LONG' ? 'text-emerald-400' : direction === 'SHORT' ? 'text-red-400' : 'text-gray-300'}`}>
                  {plan ? (direction === 'LONG' ? 'BUY BIAS' : 'SELL BIAS') : 'NEUTRAL (รอสัญญาณจากระบบ)'}
                </span>
              </div>

              <div className="flex justify-between items-center border-b border-gray-800 pb-2">
                <span className="text-gray-400">6 Canonical Agents Readiness:</span>
                <span className="text-emerald-400 font-mono font-semibold">6 / 6 Agents Ready</span>
              </div>

              <p className="text-gray-400 text-xs leading-relaxed">
                {plan
                  ? 'บทวิเคราะห์สรุป: Setup สอดคล้องกับโครงสร้างราคาสภาพคล่อง SMC และอยู่นอกกรอบความเสี่ยงของข่าวเศรษฐกิจมหภาค'
                  : 'ระบบ AI Multi-Agent พร้อมรวบรวมบริบท (Context) และวิเคราะห์เชิงให้คำปรึกษาทันทีที่มี Trade Candidate ที่ผ่านเกณฑ์จาก Strategy Engine'}
              </p>
            ...
            <div className="space-y-2.5 text-xs">
              {decisions.length > 0 ? (
                decisions.slice(0, 3).map((d) => (
                  <div key={d.id} className="border-b border-gray-800/80 pb-2">
                ...
                data.candidates.slice(0, 3).map((c) => (
                  <div key={c.id} className="border-b border-gray-800/80 pb-2">
                    <div className="flex justify-between items-center">
        ...
        <div className="bg-[#111827] border border-gray-800 rounded-xl p-5 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-2">
              {(data?.calendar_events || []).slice(0, 3).map((e: EconomicEvent) => (
                <div key={e.id} className="flex flex-wrap items-center gap-2 py-1.5 border-b border-gray-800/50 last:border-0">
