...
export class MarketConnection {
  ...
  private async connect() {
    ...
    try {
      ...
      socket.onopen = () => {
        ...
        socket.send(JSON.stringify({ type: 'subscribe', symbol: this.symbol, timeframe: this.timeframe }));
      };
      socket.onmessage = (event) => {
        try {
          const message = parseMessage(JSON.parse(event.data));
          if (message.symbol !== this.symbol || message.timeframe !== this.timeframe) return;
          this.lastMessage = Date.now();
          if (message.type === 'update' && message.sequence <= this.lastSequence) return;
          ...
          if (Date.now() - this.opened >= 10000) this.attempts = 0;
      ...
      socket.onclose = (event) => {
        ...
        this.watchdog = null;
        this.socket = null;
        if (event.code === 1008) { this.onState('ERROR'); return; }
        this.retry();
      };
  ...
  private retry() {
    if (this.stopped) return;
    if (this.attempts >= 10) { this.onState('DISCONNECTED'); return; }
    this.onState('RECONNECTING');
    this.timer = setTimeout(() => { this.timer = null; void this.connect(); }, reconnectDelay(this.attempts++));
  }
}
