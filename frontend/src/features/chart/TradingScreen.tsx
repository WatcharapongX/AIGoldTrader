...
const ChartCanvas = memo(function ChartCanvas({ bind }: { bind: (chart: IChartApi, series: ISeriesApi<'Candlestick'>) => () => void }) {
  ...
  useEffect(() => {
    ...
    const series = chart.addSeries(CandlestickSeries, {
      upColor: '#36c4a4', downColor: '#ed7a88', wickUpColor: '#36c4a4', wickDownColor: '#ed7a88',
      borderVisible: false, priceFormat: { type: 'price', precision: 2, minMove: 0.01 },
    });
    const unbind = bind(chart, series);
    return () => { unbind(); chart.remove(); };
  }, [bind]);
  return <div ref={container} className="market-chart" aria-label="XAUUSD candlestick chart" role="img" />;
});
...
export function TradingScreen() {
  ...
  const [strategyPrimitive] = useState(() => new AnalysisPrimitive());
  ...
  const [bind] = useState(() => (c: IChartApi, s: ISeriesApi<'Candlestick'>) => {
    ...
    s.attachPrimitive(strategyPrimitive);
    return () => { s.detachPrimitive(strategyPrimitive); s.detachPrimitive(primitive); chart.current = null; series.current = null; };
  });
      ...
      if (snapshot && candles.length) {
        ...
        for (const item of candles) {
          ...
          if (data.current.length > 1000) {
            ...
            series.current?.setData(data.current.map(chartPoint));
          } else series.current?.update(chartPoint(item));
        }
      }
      setCount(data.current.length);
      if (snapshot || candles.some(item => item.is_closed)) {
        setRevision(data.current.filter(item => item.is_closed).slice(-300)
          .map(item => [item.open_time, item.open, item.high, item.low, item.close, item.volume].join(',')).join(';'));
      }
    };
    const load = async () => {
      setProvider(null); setError(''); setCount(0); setQuote(null); latest.current = null;
      state('CONNECTING');
      try {
        const service = parseStatus(await api.get('/market/status', { signal: abort.signal }));
        if (!active) return;
        setProvider(service); staleSeconds.current = service.stale_after_seconds;
        // Initialization is asynchronous; bounded polling avoids showing an empty historical chart as ready.
        for (let attempt = 0; attempt < 30; attempt++) {
          const list = parseSymbols(await api.get('/symbols', { signal: abort.signal }));
          if (!active) return;
          setSymbols(list);
          if (list.some(item => item.name === symbol)) {
            const page = parseCandles(await api.get('/market/candles?symbol=' + encodeURIComponent(symbol) +
              '&timeframe=' + timeframe + '&limit=300', { signal: abort.signal }));
            if (!active) return;
            if (page.candles.length) {
              if (page.candles.some(item => item.source !== service.source)) throw new Error('Market source mismatch');
              refreshSeries(page.candles, true); break;
            }
          }
        ...
        if (active) { state('ERROR'); setError(cause instanceof Error ? cause.message : 'Market data unavailable'); }
      }
    };
    void load();
    const freshness = setInterval(() => {
      if (active && latest.current && connectionState.current === 'CONNECTED' &&
          Date.now() - instant(latest.current.timestamp) > staleSeconds.current * 1000) setStatus('STALE');
    }, 1000);
    return () => { active = false; abort.abort(); connection?.stop(); clearInterval(freshness); };
  }, [symbol, timeframe, retry]);

  const selected = symbols.find(item => item.name === symbol);
  const digits = provider?.digits ?? selected?.digits ?? 2;
  useEffect(() => {
    series.current?.applyOptions({ priceFormat: { type: 'price', precision: digits,
      minMove: provider?.tick_size ? Number(provider.tick_size) : 10 ** -digits } });
  }, [digits, provider?.tick_size]);
  const number = (value?: string) => value === undefined ? '—' : Number(value).toLocaleString('en-US', { minimumFractionDigits: digits, maximumFractionDigits: digits });
  const lastUpdate = quote ? new Date(quote.timestamp).toLocaleTimeString('en-GB', { timeZone: 'UTC', hour12: false }) + ' UTC' : 'Waiting for quote';
  return (
    <div className="trading-workspace">
      <div className="market-title-row">
        <div><p className="market-eyebrow">MARKET WORKSPACE</p><h1>Trading</h1></div>
      ...
      <section className="market-toolbar" aria-label="Market quote">
        <div className="market-symbol">
          ...
          <div><label htmlFor="market-symbol">Instrument</label>
            <select id="market-symbol" value={symbol} onChange={event => setSymbol(event.target.value)}>
              {(symbols.length ? symbols : [{ name: 'XAUUSD', source_available: true }]).map(item =>
                <option key={item.name} value={item.name} disabled={!item.source_available}>{item.name}</option>)}
            </select><small>Gold / US Dollar</small>
          </div>
        </div>
        <div className="quote-cell"><span>BID</span><strong data-testid="bid" className="bid-value">{number(quote?.bid)}</strong></div>
        <div className="quote-cell"><span>ASK</span><strong data-testid="ask">{number(quote?.ask)}</strong></div>
        <div className="quote-cell"><span>SPREAD · USD</span><strong data-testid="spread">{number(quote?.spread)}</strong></div>
        <div className="market-connection">
          <span data-testid="connection" className={'connection-label state-' + status.toLowerCase()}>● {status}</span>
      ...
      <section className="chart-panel">
        <div className="chart-toolbar">
          <div className="timeframes" role="group" aria-label="Timeframe">
            {timeframes.map(tf => <button key={tf} aria-pressed={timeframe === tf} onClick={() => setTimeframe(tf)}>{tf}</button>)}
          </div>
          <span className="chart-caption">CANDLES · BID · UTC</span>
        </div>
        <div className="chart-heading"><span>{symbol} <b> / {timeframe}</b></span><span data-testid="candle-count">{count} candles</span></div>
        <div className="chart-stage">
          <ChartCanvas bind={bind} />
          {!count && <div className="chart-overlay" role="status">{error ? 'NO DATA' : 'Loading historical candles…'}</div>}
        </div>
        <div className="chart-footer"><span>{provider?.mode === 'SIMULATED' ? 'Deterministic replay · not live market prices' : (provider?.detail || 'Verifying market source…')}</span>
          <a href="https://www.tradingview.com/" target="_blank" rel="noreferrer">Charts by TradingView</a></div>
      </section>
      {(error || ['ERROR', 'DISCONNECTED', 'RECONNECTING', 'STALE'].includes(status)) &&
        <div className="market-notice" role="status"><span>{error || (status + ' — waiting for fresh market data.')}</span>
          <button onClick={() => setRetry(value => value + 1)}>Reconnect</button></div>}
      <StrategyWorkspace timeframe={timeframe} source={provider?.source || ''} revision={revision} primitive={strategyPrimitive} />
      <TradingNewsPanel />
      <AnalysisWorkspace symbol={symbol} timeframe={timeframe} source={provider?.source || ''} revision={revision}
