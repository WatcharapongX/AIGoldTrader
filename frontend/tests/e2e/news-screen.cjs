...
async function main(){
 ...
 try{
  ...
  await page.evaluate(async()=>{const token=localStorage.getItem('access_token');if(token)await fetch('http://localhost:8000/api/auth/logout',{method:'POST',headers:{Authorization:'Bearer '+token}}).catch(()=>{});}).catch(()=>{});
