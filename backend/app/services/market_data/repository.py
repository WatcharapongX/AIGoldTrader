    ...
    for offset in range(0, len(candles), 100):
        ...
        stmt = stmt.on_conflict_do_update(
            ...
            set_={c.name: getattr(stmt.excluded, c.name)
                  for c in MarketCandle.__table__.columns if c.name not in columns},
        )
...
async def save_tick(session, symbol_id: uuid.UUID, tick: Tick) -> None:
    insert = pg_insert if session.bind.dialect.name == "postgresql" else sqlite_insert
    await session.execute(insert(MarketTick).values(
        symbol_id=symbol_id, source=tick.source, ts=tick.timestamp,
        bid=tick.bid, ask=tick.ask, spread=tick.ask - tick.bid, volume=tick.volume,
...
async def prune(session, symbol_id: uuid.UUID, now: dt.datetime) -> None:
    ...
    await session.execute(delete(MarketTick).where(
        MarketTick.symbol_id == symbol_id, MarketTick.source == "simulated",
        MarketTick.ts < now - dt.timedelta(days=1),
    ))
    for timeframe, seconds in SECONDS.items():
        await session.execute(delete(MarketCandle).where(
            MarketCandle.symbol_id == symbol_id, MarketCandle.source == "simulated",
            MarketCandle.timeframe == timeframe.value,
            MarketCandle.bucket_start < now - dt.timedelta(seconds=seconds * 1000),
        ))


async def candles_query(session, symbol_id, symbol, timeframe, start=None, end=None, limit=300,
                        source="simulated") -> list[Candle]:
    query = select(MarketCandle).where(
        MarketCandle.symbol_id == symbol_id, MarketCandle.source == source,
        MarketCandle.timeframe == timeframe.value,
    )
    if start:
    ...
    return [Candle(
        symbol=symbol, timeframe=Timeframe(row.timeframe),
        open_time=row.bucket_start.replace(tzinfo=dt.UTC) if row.bucket_start.tzinfo is None else row.bucket_start,
        open=row.open, high=row.high, low=row.low, close=row.close, volume=row.volume,
        bid_close=row.bid_close, ask_close=row.ask_close, source=row.source, is_closed=row.is_closed,
